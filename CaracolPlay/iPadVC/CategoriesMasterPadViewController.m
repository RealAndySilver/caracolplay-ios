...
-(UIActivityIndicatorView *)spinner {
    if (!_spinner) {
        ...
        _spinner.frame = CGRectMake(160 - 20, 384 - 20.0, 40.0, 40.0);
    }
    return _spinner;
}
...
-(void)setupUI {
    ...
    //Create a bar button to reload the content
    UINavigationItem *navigationItem = [[UINavigationItem alloc] initWithTitle:nil];
    self.navigationBar.items = @[navigationItem];
    UIBarButtonItem *reloadBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh target:self action:@selector(getCategories)];
...
-(void)getCategories {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"GetCategoriesNotification" object:nil];
